...
class FileUploader():
  ...
  def remove(self, selector, callback = None):
    client.call("_FilesCollectionRemove_" + self.collectionName, selector, self._remove_callback)
  ...
  def _upload_start_callback(self, error, result):
    if error:
      self.error = True
      print(error)
      return
    try:
      with open(self.filePath, "rb") as _file:
        for i in xrange(self.chunkCount):
          ...
          client.call(self.methodNames['_Write'], [opts], self._upload_write_callback)
          #print requests.post('http://localhost:3100/cdn/storage/Images/__upload', data= {'data':opts})
    except Exception as e:
      print(e)
      self.error = True
      client.call(self.methodNames['_Abort'], [self.fileId])
    else:
      opts = {
      "eof": True,
      "fileId": self.fileId,
      }
      client.call(self.methodNames['_Write'], [opts], self._upload_end_callback)
